...
type Note = {
  ...
  anchor?: {imgX: number; imgY: number}; // ← NUEVO
};
...
const ZoomScreen: React.FC<Props> = (props) => {
  ...
  const getFittedSize = () => {
    const imgW = originalImageWidth.current || width; // fallback
    const imgH = originalImageHeight.current || deviceHeight; // fallback
    const r = width / (imgW || 1);
    return {width, height: imgH * r};
  };

  const getDiffPositionY = () => {
    const {height: fittedH} = getFittedSize();
    return (dimensionsHeight - fittedH) / 2;
  };
  ...
  const updateMainImageTransform = useCallback(
    (position: IOnMove) => {
      ...
      // ... tu cálculo de offX/offY/diffY (globalPositionX/Y y getDiffPositionY)
      const S = zoomScale.current;

      setNotes((prev) => {
        const next = prev.map((n) => {
          if (!n.areaSet || !n.position.scale || !n.anchor) return n;

          const S0 = n.position.scale;
          const finalLeft = n.anchor.imgX * S - globalPositionX.current;
          const finalTop =
          ...
          return {
            ...n,
            // No tocar position.top/left
            width: finalWidth,
            height: finalHeight,
            translation: {top: finalTop, left: finalLeft},
            stickyNoteTranslation: {
              ...(n.stickyNoteTranslation ?? {
                absoluteX: 0,
                absoluteY: 0,
                x: 0,
                y: 0,
  ...
  //     const imgW = (originalImageWidth.current ||= dimensionsWidth);
  //     const imgH = (originalImageHeight.current ||= deviceHeight);
    ...
    ({note, measure}: any) => {
      setNotes((prev) =>
        prev.map((n) => {
          ...
          const S = zoomScale.current || 1;
          const offX = globalPositionX.current;
          const offY = globalPositionY.current;
          const diffY = getDiffPositionY();

          const tLeft = measure.translation.left;
          const tTop = measure.translation.top;

          const seededAnchor = {
            imgX: (tLeft + offX) / S,
            imgY: (tTop + offY - diffY) / S,
          };

          return {
            ...n,
            ...measure, // width/height/translation/originalWidth/originalHeight
            diffLeft: n.position.left - measure.translation.left,
            diffTop: n.position.top - measure.translation.top,
            screenWidth: width,
            screenHeight: deviceHeight,
            areaSet: true,
            updating: false,
            position: n.position, // 👈 NO reescribas position
            anchor: n.anchor ?? seededAnchor, // si no hay, siémbralo
          };
        }),
      );
    },
      ...
      if (buttonName === 'set') {
        setNotes((prev) =>
          prev.map((n) => {
            ...
            return {
              ...n,
              position: {...n.position, scale: S},
              anchor, // ⬅⬅ SIEMPRE lo sobrescribimos
              updating: false,
              areaSet: true,
            };
          }),
