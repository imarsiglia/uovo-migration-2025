...
export function AutocompleteContext<TField extends FieldValues>({
  ...
  // Flag para saber si el usuario está escribiendo (evita sincronías invasivas)
  ...
  // pero NO mientras el usuario está escribiendo.
  useEffect(() => {
    if (!dropdownRef.current) return;

    if (typingRef.current) {
      // Si estamos escribiendo, NO imponemos selección ni limpiamos el input.
      return;
    }
  ...
  const handleSelect = useCallback(
    (item: AnyItem | null) => {
      // El usuario eligió un item -> ya no estamos escribiendo
  ...
  // Wrap de onChangeText/onFocus/onBlur para controlar el typing y limpiar form
  const {
    onChangeText: onChangeTextProp,
    onFocus: onFocusProp,
    onBlur: onBlurProp,
    ...restProps
  } = rest as IAutocompleteDropdownProps;
  ...
  const handleChangeText = useCallback(
    (text: string) => {
      ...
      // Si había algo seleccionado en el form, lo limpiamos
      // (así selectedId pasa a null y no se reimpone el valor anterior)
      if (field.value != null) {
        field.onChange(null);
      }
  ...
  const handleFocus = useCallback(() => {
    typingRef.current = false; // recién focus, aún no se escribe
    // @ts-ignore
    onFocusProp?.();
  }, [onFocusProp]);

  const handleBlur = useCallback(() => {
    typingRef.current = false; // se dejó de escribir
    // @ts-ignore
    onBlurProp?.();
  }, [onBlurProp]);
  ...
  return (
    <CustomAutocomplete
      ...
      // Si el usuario limpia manualmente desde el clear del dropdown
      onClear={() => {
        typingRef.current = false;
        field.onChange(null);
      }}
