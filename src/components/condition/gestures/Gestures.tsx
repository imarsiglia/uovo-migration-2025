// From https://github.com/keske/react-native-easy-gestures
...
// Utils
import {angle, distance} from './maths.js';
import {getAngle, getScale, getTouches, isMultiTouch} from './events.js';
...
export default class Gestures extends Component {
  ...
  constructor(props) {
    ...
    this.pan = PanResponder.create({
      ...
      onMoveShouldSetPanResponderCapture: (event, {dx, dy}) => {
        return Math.abs(dx) > 1.5 || Math.abs(dy) > 1.5;
      },
  ...
  onDrag(event, gestureState) {
    ...
    const RR = require('ramda');
    const isObject = RR.is(Object, draggable);
  ...
  onScale = (event) => {
    ...
    const RR = require('ramda');
    const isObject = RR.is(Object, scalable);
