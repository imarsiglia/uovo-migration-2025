...
export const writeQueue = (q: OutboxItem[]) => store.set(KEY, JSON.stringify(q));
...
export const enqueueCoalesced = (kind: OutboxKind, payload: NoteType) => {
  ...
  // Normalizar payload: prefer clientId when create
  // Buscamos si existe item relacionado (por clientId o by server id)
  const matchFilter = (it: OutboxItem) => {
    const p = it.payload as NoteType;
    if (payload.clientId && p.clientId) return p.clientId === payload.clientId;
    if (payload.id && p.id) return p.id === payload.id;
    // fallback: same job + same title? (risky) -> avoid
    return false;
  };

  // 1) try find existing related items
  const idx = q.findIndex(matchFilter);

  if (idx === -1) {
    // nada relacionado: simplemente push
  ...
  // 2) hay item relacionado: coalesce según reglas
