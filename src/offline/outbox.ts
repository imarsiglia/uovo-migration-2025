...
const KEY_SESSION = 'OUTBOX_SESSION_V16';
const KEY_FAILED_ARCHIVE = 'OUTBOX_FAILED_ARCHIVE_V14';
...
export async function getQueueByStatus(): Promise<
  Record<OutboxStatus, OutboxItem[]>
> {
  const q = await readQueue();
  return {
...
export async function readFailedArchive(): Promise<FailedArchiveItem[]> {
  ...
  if (!raw) return [];
  try {
    return JSON.parse(raw) as FailedArchiveItem[];
  } catch {
    return [];
  }
}
...
export async function clearProcessingSession() {
  await d(KEY_SESSION);
}
...
export async function enqueueCoalesced(
  ...
  const matches = q
    .map((it, i) => ({it, i}))
    .filter(({it}) => sameRecord(it.payload, payload));
